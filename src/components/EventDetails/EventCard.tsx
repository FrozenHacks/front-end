...
import { RootState, TPricing } from "../../types/Slices";
...
const EventBookCard = () => {
  ...
  const [currentScheme, setCurrentScheme] = useState<TPricing>({
    _id: "",
    title: "",
    desc: "",
    amount: 0,
  ...
  const PayNow = async (e: React.MouseEvent) => {
    try {
      ...
      const resp = await axios({
        url: `${BASEURL}/transactions/event/create`,
        method: "POST",
        headers: {
      ...
      if (resp.data) {
        ...
        const url = resp.data.transactionDetails;
        const windowName = "HitPay - Checkout";
    ...
    <div className="z-20 flex flex-col w-full mt-5">
      ...
      <div className="sm:mt-8 mt-4 text-white">
        ...
        <div className="flex justify-between py-2 text-sm">
          ...
          <span>SGD {currentScheme.amount}</span>
        </div>
        <div className="flex justify-between py-2 text-sm">
          <span className="flex items-center space-x-3">
            <svg
        ...
        <div className="flex justify-between py-4 mt-4 border-t">
          ...
          <span className="text-lg font-semibold">
            {currentScheme.amount * 0.18 + currentScheme.amount}
          </span>
        </div>
